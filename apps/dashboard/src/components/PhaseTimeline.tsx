...
export function PhaseTimeline({ events }: Props) {
  if (events.length === 0) {
    return <p className="text-xs italic text-slate-400">no phases recorded</p>;
  }
  ...
  return (
    <ul className="space-y-1">
      {Array.from(latestByPhase.values()).map((e) => (
        <li
          key={e.phase}
          className="flex items-center justify-between gap-3 rounded-md bg-white px-3 py-1.5 text-xs ring-1 ring-slate-200"
        >
          <span className="font-mono text-slate-700">{e.phase}</span>
          <span className={`rounded px-2 py-0.5 ${STATUS_STYLES[e.status]}`}>
            {e.status}
